...
// A day-level net-P&L override (`Day.pnl_override`) REPLACES the sum of that
// day's trade P&Ls in every money/equity statistic. These helpers are the
// single place that rule lives, so every aggregation site folds overrides in
// the same way instead of each re-deriving it.

/**
 * Net realised P&L per day for a set of trades, with override days replaced.
 ...
 * result, so callers that sum the map pick up their P&L.
 */
